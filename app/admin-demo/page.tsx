...
import { OPENAI_KEY_HEADER, OPENAI_SESSION_KEY } from "@/lib/openai-config";
...
export default function Page(){
  ...
  async function testConnection(candidate:string, storeOnSuccess:boolean){
    ...
    try{
      const r=await fetch("/api/admin/test-openai",{method:"POST",headers:{"x-common-admin-pin":pin,[OPENAI_KEY_HEADER]:candidate}});
      ...
      if(storeOnSuccess)sessionStorage.setItem(OPENAI_SESSION_KEY,candidate);
  ...
  function disconnect(){sessionStorage.removeItem(OPENAI_SESSION_KEY);setConnection("disconnected");setStatusTitle("AI 연결 안 됨");setKey("");setMessage("")}
