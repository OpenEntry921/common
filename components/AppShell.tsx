...
export function AppShell({children}:{children:React.ReactNode}) {
 ...
 {!minimal&&<><footer><div><Link className="brand footer-brand" href="/"><b>{SITE_CONFIG.serviceName}</b><small>{SITE_CONFIG.tagline}</small></Link><p>서로의 일상에 작은 도움이 되는 동네 커뮤니티</p></div><div className="footer-links"><Link href="/impact">Community Impact</Link><Link href="/privacy">개인정보와 신뢰</Link><Link href="/church">이 공간을 운영하는 곳</Link><Link href="/admin-demo">Admin Demo</Link></div><small>Hosted by {SITE_CONFIG.churchName}</small></footer>
