...
export function AppShell({children}:{children:React.ReactNode}) {
 ...
 {open&&<div className="drawer-backdrop" onClick={()=>setOpen(false)}><aside className="drawer" onClick={e=>e.stopPropagation()}><button className="drawer-close" onClick={()=>setOpen(false)} aria-label="닫기"><X/></button><p className="eyebrow">EXPLORE COMMON</p>{nav.map(n=><Link key={n.href} href={n.href} onClick={()=>setOpen(false)}>{n.label}<span>→</span></Link>)}<hr/><Link href="/market" onClick={()=>setOpen(false)}>나눔 장터<span>→</span></Link><Link href="/impact" onClick={()=>setOpen(false)}>Community Impact<span>→</span></Link><Link href="/church" onClick={()=>setOpen(false)}>이 공간을 운영하는 곳<span>→</span></Link><a href={SITE_CONFIG.churchUrl} target="_blank" rel="noreferrer" onClick={()=>setOpen(false)}>{SITE_CONFIG.churchName} 알아보기<span>↗</span></a><Link href="/privacy" onClick={()=>setOpen(false)}>개인정보와 신뢰<span>→</span></Link></aside></div>}
 </>;
}
