...
export default function Page(){
  ...
  </div><div className="inline-actions"><button className="button" type="button" onClick={diagnose} disabled={diagnosing}>{diagnosing?"진단 중…":aiStatus?.openaiConnection?"다시 진단":"AI 연결 진단"}</button><Link className="button secondary" href="/heart/letter">마음편지 확인</Link></div><p className="billing-note">진단은 서버에서 직접 기본 연결과 실제 마음편지 Structured Output을 각각 검사합니다. 비밀 키와 사용자 내용은 표시하지 않습니다.</p></section><section className="card ai-admin-card"><p className="eyebrow">PRAYER REQUEST</p><h2>기도 요청 설정</h2><p className={`connection-status ${aiStatus?.prayerEmail?.configured?"on":""}`}>기도 요청 이메일　● {aiStatus?.prayerEmail?.configured?"설정됨":"이메일 발송 설정 필요"}</p><p><strong>수신 이메일</strong>　{aiStatus?.prayerEmail?.recipient||"PRAYER_RECIPIENT_EMAIL 설정 필요"}</p><p><strong>교회 홈페이지</strong>　<a className="church-link" href={SITE_CONFIG.churchUrl} target="_blank" rel="noreferrer">{SITE_CONFIG.churchName} ↗</a></p><p className="billing-note">수신 주소는 Netlify의 PRAYER_RECIPIENT_EMAIL 환경변수에서 변경합니다.</p></section><span className="demo-label">DEMO DASHBOARD · 실제 운영 데이터가 아닙니다</span><div className="grid admin-grid" style={{marginTop:24}}>{stats.map(([n,l,x])=><article className="card admin-card" key={l}><small>{l}</small><b>{n}</b><small>{x}</small></article>)}</div><div style={{textAlign:"center",marginTop:70}}><Link href="/impact" className="button">Community Impact 보기</Link> <Link href="/" className="button secondary">사용자 화면 보기</Link></div></div></section></>;
}
